...
docker run --network=host -it --rm -e DISPLAY=$DISPLAY -e VACA_PREFIX=$VACA_PREFIX -e EPICA_CA_AUTO_ADDR_LIST=$EPICS_CA_AUTO_ADDR_LIST -e EPICS_CA_ADDR_LIST="$EPICS_CA_ADDR_LIST" -e PYDM_DEFAULT_PROTOCOL=ca -v /tmp/.X11-unix:/tmp/.X11-unix lnls/fac-hla /bin/bash -c $HLA
